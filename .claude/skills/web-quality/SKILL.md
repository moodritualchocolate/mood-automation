---
name: web-quality
description: Comprehensive web quality audit and optimization across Core Web Vitals (LCP, INP, CLS), performance/loading speed, accessibility (WCAG 2.2), SEO, and best practices, based on Google Lighthouse guidelines. Orchestrates focused sub-skills for each category. Use when asked to "audit my site", "review web quality", "run lighthouse audit", "check page quality", "optimize my website", "improve Core Web Vitals / LCP / CLS / INP", "speed up my site", "accessibility / a11y / WCAG review", or "SEO audit".
license: MIT
metadata:
...
For detailed guidelines on specific areas, read the nested sub-skills:
- [Performance Optimization](skills/performance/SKILL.md)
- [Core Web Vitals](skills/core-web-vitals/SKILL.md)
- [Accessibility](skills/accessibility/SKILL.md)
- [SEO](skills/seo/SKILL.md)
- [Best Practices](skills/best-practices/SKILL.md)

## Automated static scan (optional)

A read-only HTML analyzer ships with this skill. It flags missing doctype/viewport/
lang/title/charset, images without alt, non-descriptive link text, and similar issues
as structured JSON (no filesystem mutations). Requires `jq`.

```bash
bash scripts/analyze.sh <file-or-directory>
```

Use it as a first pass, then apply the category guidance above for anything it can't
detect statically (runtime CWV, contrast, etc.).
