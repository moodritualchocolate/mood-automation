/**
 * GET /api/campaign-evolution
 *
 * Read-only campaign lifecycle view over data/memory/campaign-evolution-memory.json.
 * Returns a CampaignLifecycleLongitudinalView shaped for the studio
 * CampaignEvolutionPanel. The "current" reading is computed from
...
import {
  createCampaignLifecycleMemoryStore, buildCampaignLifecycleHistoryContext,
...
export async function GET(req: NextRequest) {
  ...
  const [
    lifecycleMem, counterfactualMem, outcomeMem, governanceMem, identityMem, weightMem,
  ...
  const current = computeCampaignEvolution({
    culturalPerception,
    identityContinuity: identity,
    executiveGovernance: governance,
    strategicOutcome: outcome,
    ...
    policyAuditView,
    qualityLongitudinal,
    history: buildCampaignLifecycleHistoryContext(lifecycleMem),
  });

  const view = buildCampaignLifecycleLongitudinalView({
    memory: lifecycleMem,
    current,
  });
