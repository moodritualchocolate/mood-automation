/**
 * GET /api/scene-architect
 *
 * Scene Architect observatory. Composes story architect output +
 * memory imprint + human presence + world model + creative director
 * into exploratory SCENE BLUEPRINTS.
 *
 * STRICT CONTRACT:
...
export async function GET(): Promise<NextResponse> {
  ...
  // Recompose all upstream layers (self-contained route).
  ...
  const reading = computeSceneArchitect({
  ...
  return NextResponse.json({
    ...
    dominantSceneTypes: reading.dominantSceneTypes,
    notes: reading.notes,
    advisoryNotice:
