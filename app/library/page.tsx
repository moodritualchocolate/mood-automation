...
function LibraryInner() {
  ...
  // Full kit as plain text (roadmap #13) — for copy-all.
  function kitAsText(): string {
    if (!data?.library) return '';
    const { chosenOneLiner, hooks, ugcScripts, imageConcepts } = data.library;
    const L: string[] = [];
    L.push('═══ YOUR BRAND · ONE SENTENCE ═══');
    if (chosenOneLiner) L.push(chosenOneLiner.text);
    L.push('', `═══ HOOKS (${hooks.length}) ═══`);
    hooks.forEach((h, i) => {
      L.push(`${i + 1}. ${h.text}`);
      L.push(`   audience: ${h.audience}`);
      L.push(`   situation: ${h.situation}`);
      L.push(`   visual: ${h.visualDirection}`, '');
    });
    L.push(`═══ UGC SCRIPTS (${ugcScripts.length}) ═══`);
    ugcScripts.forEach((u) => {
      L.push(`● ${u.title} · ${u.durationSec}s`);
      L.push(u.scriptHebrew);
      L.push(`   shots: ${u.shotList.join(' → ')}`);
      L.push(`   CTA: ${u.callToActionHebrew}`, '');
    });
    L.push(`═══ IMAGE CONCEPTS (${imageConcepts.length}) ═══`);
    imageConcepts.forEach((c) => {
      L.push(`● ${c.title}`);
      L.push(`   ${c.visualDescription}`);
      L.push(`   render: ${c.renderingNote}`, '');
    });
    L.push('— Generated by MOOD Creative OS · operator-approved · human remains final authority.');
    return L.join('\n');
  }

  // Downloadable brief (roadmap #13) — a self-contained print-ready
  // HTML file the customer can hand to a team or save as PDF.
  function exportHtml() {
    if (!data?.library) return;
    const { chosenOneLiner, hooks, ugcScripts, imageConcepts } = data.library;
    ...
    const html = [
      ...
      '<title>Creative Kit · MOOD Creative OS</title>',
      '<style>body{font-family:Georgia,"Times New Roman",serif;max-width:760px;margin:40px auto;padding:0 24px;color:#1a1a1a;line-height:1.6}h1{font-size:34px;letter-spacing:-0.02em}h2{font-size:15px;text-transform:uppercase;letter-spacing:0.2em;color:#888;margin-top:44px;border-bottom:1px solid #ddd;padding-bottom:8px}.hook{font-size:20px;margin:18px 0 4px}.meta{font-size:12px;color:#777}.script{white-space:pre-wrap;background:#f6f5f2;padding:14px;border-radius:8px;font-size:14px}.foot{margin-top:60px;font-size:11px;color:#999}@media print{body{margin:16px auto}}</style>',
      '</head><body>',
      '<div style="font-size:11px;letter-spacing:0.3em;text-transform:uppercase;color:#999">MOOD Creative OS · Creative Kit</div>',
      chosenOneLiner ? `<h1>${esc(chosenOneLiner.text)}</h1>` : '',
      `<h2>Hooks · ${hooks.length}</h2>`,
      ...hooks.map((h, i) => `<div class="hook">${i + 1} · ${esc(h.text)}</div><div class="meta">${esc(h.audience)} · ${esc(h.situation)}<br>${esc(h.visualDirection)}</div>`),
      `<h2>UGC Scripts · ${ugcScripts.length}</h2>`,
      ...ugcScripts.map((u) => `<div class="hook">${esc(u.title)} · ${u.durationSec}s</div><div class="script">${esc(u.scriptHebrew)}</div><div class="meta">Shots: ${esc(u.shotList.join(' → '))} · CTA: ${esc(u.callToActionHebrew)}</div>`),
      `<h2>Image Concepts · ${imageConcepts.length}</h2>`,
      ...imageConcepts.map((c) => `<div class="hook">${esc(c.title)}</div><div class="meta">${esc(c.visualDescription)}<br>Render: ${esc(c.renderingNote)}</div>`),
      '<div class="foot">Operator-approved · no auto-publishing · human remains final authority.</div>',
      '</body></html>',
    ].join('\n');
    const blob = new Blob([html], { type: 'text/html;charset=utf-8' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'creative-kit.html';
    a.click();
    URL.revokeObjectURL(a.href);
  }

  // Ready-to-paste AI image prompt (roadmap #29 · prompt-payload level).
  function imagePrompt(c: ImageConcept): string {
    return [
      `${c.visualDescription}.`,
      `Style: ${c.renderingNote}.`,
      'Photorealistic, editorial documentary photograph. No text overlay, no watermarks, no logos.',
      '--ar 4:5',
    ].join(' ');
  }
      ...
      <PageHead
        ...
        actions={
          <>
            ...
            <Button variant="ghost" size="md" onClick={exportHtml}>
              ↓ Export brief
            </Button>
            <Link href="/onboard"><Button variant="secondary" size="md">+ New kit</Button></Link>
          </>
        }
      />
              ...
              <Card key={c.id}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="flex items-center gap-3">
                    <button
                      onClick={() => copy(imagePrompt(c), `imgp-${c.id}`)}
                      className="text-[11px] text-[#C9A24B] hover:text-[#F7F5F2]"
                      title="Paste into Midjourney / DALL·E / Ideogram"
                    >
                      {copied === `imgp-${c.id}` ? 'Copied ✓' : '✦ AI image prompt'}
                    </button>
                    <button
